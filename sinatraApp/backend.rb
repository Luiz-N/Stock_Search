...
get '/companies/:symbol' do |symbol|
	...
	company = settings.companies.find({_id: symbol}).projection({_id: 0})
		.find_one_and_update(
			"$set" => { :last_update => Date.today.to_time.to_i }
		)

	if company[:last_update] != Date.today.to_time.to_i
		company[:quotes] = get_latest_quotes(company)
